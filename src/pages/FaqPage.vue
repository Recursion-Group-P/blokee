        ...
        <q-tab-panels v-model="tab" animated>
          <q-tab-panel name="game-rules">
            <div v-for="rule in gameRules" :key="rule.ruleId">
              <h3 class="text-h5 dark q-py-sm">{{ rule.heading }}</h3>
              <div v-for="d in rule.details" :key="d" class="dark">
                <p class="q-ml-md">{{ d }}</p>
              </div>
            </div>
          </q-tab-panel>
          ...
          <q-tab-panel name="faq">
            ...
            <!-- Google Play Storeに公開したらコメントあうとしてリンクを付ける  -->
            <!-- <div class="q-pa-sm">
              <q-expansion-item
                class="shadow-1 overflow-hidden"
                style="border-radius: 4px"
                icon="help"
                label="他の端末（プラットフォーム）でもプレイ可能ですか？"
                header-class="bg-dark text-white"
                expand-icon-class="text-white"
              >
                <q-card>
                  <q-card-section>
                    <div>
                      はい。BlockeeはQuasarフレームワークを使用して作成された、ウェブ、モバイル、デスクトップを含むマルチプラットフォームなアプリケーションです。
                      <hr />
                      <p>
                        Androidでプレイする：<a href="#">Google Play Store Link</a> <br />
                        MacまたはWindowsでプレイする：<a href="#">Desktop App Download Link</a>
                      </p>
                    </div>
                  </q-card-section>
                </q-card>
    ...
    return {
      ...
      gameRules: [
        ...
        },
        {
          ...
          heading: 'スコア計算',
          details: [
            '置いたピースのマスの数がスコアに加算されます。',
            '最後に置いたピースのマスが1つの場合、ボーナスポイント（+5pt）が加算されます。',
            '全てのピースを使い切るとボーナスポイント（+15pt）が加算されます。',
          ],
        },
        {
          ruleId: 5,
...
h3:after {
  ...
  content: '';
  border-radius: 2px;
  background-image: linear-gradient(to left, #fff 0%, #000000 100%);
}
</style>
